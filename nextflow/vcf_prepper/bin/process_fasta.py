...
def ungzip_fasta(fasta_dir: str, compressed_fasta: str) -> str:
    if os.path.dirname(compressed_fasta) != fasta_dir:
        print(
            f"[ERROR] Fasta file {fasta_dir} in wrong directory; should be in - {fasta_dir}"
        )
        exit(1)

    process = subprocess.run(
        ["gzip", "-df", compressed_fasta],
...
def bgzip_fasta(fasta_dir: str, unzipped_fasta: str) -> str:
    ...
    process = subprocess.run(
        ["bgzip", "-f", unzipped_fasta], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def index_fasta(zipped_fasta: str, force: str = False) -> None:
    ...
    if process.returncode != 0:
        print(
            f"[ERROR] Cannot index fasta file - {zipped_fasta}\n{process.stderr.decode()}\nExiting ..."
        )
        exit(1)
