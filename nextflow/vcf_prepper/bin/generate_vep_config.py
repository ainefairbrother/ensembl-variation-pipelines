...
def format_gnomad_args(source: str, metadata: dict) -> str:
    ...
    for chromosome in chromosomes:
        file = os.path.join(
            metadata["directory"],
            metadata["file_pattern"].replace("##CHR##", chromosome),
        )
    ...
    for source in FREQUENCIES:
        if source.startswith("gnomAD"):
            if assembly in FREQUENCIES[source]:
                frequencies.append(
                    format_gnomad_args(source, FREQUENCIES[source][assembly])
                )
                ...
                assembly_acc = (
                    species.split("_")[2].replace("gca", "GCA_").replace("v", ".")
                )
                file = os.path.join(
                    metadata["directory"].replace("##ASSEMBLY##", assembly_acc),
                    metadata["file_pattern"].replace("##ASSEMBLY##", assembly_acc),
                )
